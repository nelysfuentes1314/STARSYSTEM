...
import { X, Award, Star, Heart, Zap, Smile } from 'lucide-react';

const MedalModal = ({ isOpen, onClose, onAwardMedal, studentName }) => {
    ...
    const handleConfirm = () => {
        if (selectedType === 'behavior') {
            onAwardMedal({ name: 'Behavior', icon: '⭐' });
        } else if (selectedType === 'participation') {
            onAwardMedal({ name: 'Participation', icon: '✋' });
        } else if (selectedType === 'custom') {
            if (!customName.trim()) return;
            onAwardMedal({ name: customName, icon: customIcon });
        }
            ...
            <div className="bg-white border-4 border-blue-900 rounded-xl shadow-[8px_8px_0px_0px_rgba(30,58,138,1)] max-w-sm w-full overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 slide-in-from-bottom-4 duration-200">
                ...
                <div className="bg-blue-900 text-white p-4 flex justify-between items-center border-b-4 border-blue-950">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-xl font-bold font-mono tracking-tight text-yellow-400 uppercase">Award Medal</h2>
                    </div>
